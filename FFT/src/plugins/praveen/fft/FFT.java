...
public class FFT extends EzPlug {
	...
	EzVarText	swap = new EzVarText("Swap Quadrants?", new String[] { "Yes", "No" }, 0, false);
	...
	private Sequence FFT_2D(Sequence sequence, String swap, String scaling) 
	{
		...
		fImage.setName("Fourier Transformed");
			...
			try
			{
				if(scaling=="Linear scale")
					...
					for(int x = 0; x < _w; x++)
					{
						for(int y = 0; y < _h; y++)
						{
							resultArray.setDataAsDouble(x, y, 0, Math.sqrt(Math.log(Math.pow(fArray[(x + y * _h)*2 + 0], 2) + Math.pow(fArray[(x + y * _h)*2 + 1], 2))));
						}
					}
				/*for(int x = 0; x < (wc+1); x++)
				{
					...
					{
						resultArray.setDataAsDouble(x, y, 0, Math.sqrt(Math.pow(fArray[((wc-x) + (_h+hc-y) * _h)*2 + 0], 2)+Math.pow(fArray[((wc-x) + (_h+hc-y) * _h)*2 + 1], 2)));
					}
				...
				for(int x = (wc+1); x < _w; x++)
				{
					for(int y = 0; y < (hc+1); y++)
					{
						resultArray.setDataAsDouble(x, y, 0, Math.sqrt(Math.pow(fArray[((_w+wc-x) + (hc-y) * _h)*2 + 0], 2)+Math.pow(fArray[((_w+wc-x) + (hc-y) * _h)*2 + 1], 2)));
					}
					for(int y = hc+1; y < _h; y++)
					{
						resultArray.setDataAsDouble(x, y, 0, Math.sqrt(Math.pow(fArray[((_w+wc-x) + (_h+hc-y) * _h)*2 + 0], 2)+Math.pow(fArray[((_w+wc-x) + (_h+hc-y) * _h)*2 + 1], 2)));
					}
				}		*/		
